...
void traversal(BT *t){
    ...
    while(q.size()>0)[
        ...
        for(i=0;i<head->childlist.size;i++){
            ...
            head->childlist[i]->tracersal_id=j;
            j++;           
...
void splid_child(BT* t, BT** root){
    ...
    if(t->isroot==1){
        ...
        t->parent->keylist.insert(t->PCrelation-1,t->keylist[t->degree-1])
        
        right=malloc(sizeof(BT));
        init(right,t->degree,0,t->isleaf);
        t->parent->childlist.insert(t->PCrelation+1,right);
        ...
        for(i=right->PCrelation+1;i<=childlist.size();i++){
